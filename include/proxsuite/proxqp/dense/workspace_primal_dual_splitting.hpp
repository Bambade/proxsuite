...
struct SocpModel
{
   ...
   */
  SocpModel(isize _dim, isize _n_in)
    : dim(_dim),
	  n_in(_n_in),
    H(_dim, _dim),
    g(_dim),
...
struct PrimalDualSplittingWorkspace {
   ...
   */
  PrimalDualSplittingWorkspace(isize dim, isize n_in)
    : //
    ...
    , x_prev(dim)
    , y_prev(n_in)
    , kkt(dim + n_in, dim + n_in)
    , rhs(dim + n_in)
    , err(dim + n_in)
    , dw(dim + n_in)
    , dual_residual_scaled(dim)
    , primal_residual_scaled(n_in)
    , dirty(false)
    , refactorize(false)
    , proximal_parameter_update(false)

  {
    ldl.reserve_uninit(dim + n_in);
    ldl_stack.resize_for_overwrite(
      proxsuite::linalg::veg::dynstack::StackReq(

        proxsuite::linalg::dense::Ldlt<T>::factorize_req(dim + n_in) |

        (proxsuite::linalg::dense::temp_vec_req(
           proxsuite::linalg::veg::Tag<T>{}, n_in) & 
         proxsuite::linalg::veg::dynstack::StackReq{
           isize{ sizeof(isize) } * (n_in), alignof(isize) } &

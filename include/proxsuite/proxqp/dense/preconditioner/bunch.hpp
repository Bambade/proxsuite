 ...
 * @file bunch.hpp
 */
#ifndef PROXSUITE_PROXQP_DENSE_PRECOND_BUNCH_HPP
#define PROXSUITE_PROXQP_DENSE_PRECOND_BUNCH_HPP
  ...
  proxsuite::linalg::veg::dynstack::DynStackMut stack)
{
  ...
  isize n = qp.H.rows;
  for (isize i = 0; i < n; ++i) {
    T current_max(sqrt(std::abs(H(i, i))));
    for (isize j = 0; j < i; ++j) {
      current_max = std::max(current_max, delta(j) * H(i, j));
    }
    if (current_max != 0.) {
      delta(i) = 1. / current_max;
    }
  }
  ...
  for (isize i = 0; i < n_eq; ++i) {
    T current_max(0);
    for (isize j = 0; j < n; ++j) {
      current_max = std::max(current_max, delta(j) * A(i, j));
    }
    if (current_max != 0.) {
      delta(n + i) = 1. / current_max;
    }
  }
  ...
  for (isize i = 0; i < n_in; ++i) {
    T current_max(0);
    for (isize j = 0; j < n; ++j) {
      current_max = std::max(current_max, delta(j) * C(i, j));
    }
    if (current_max != 0.) {
      delta(n + n_eq + i) = 1. / current_max;
    }
  }
  // normalize A and C
...
struct BunchEquilibration
{
   ...
   */
  explicit BunchEquilibration(isize dim_,
                              isize n_eq_in,
  ...
  /*!
   * Prints Bunch equilibrator scaling variables.
   */
  void print() const
  {
  ...
  {
    return proxsuite::linalg::dense::temp_vec_req(tag, 0);
  }
  ...
  // b_new = tail @ b
  /*!
   * Scales the qp performing the bunch equilibrator algorithm considering user
   * options.
                         ...
                         proxsuite::linalg::veg::dynstack::DynStackMut stack)
  {
    if (execute_preconditioner) {
      detail::bunch_scale_qp_in_place(
        { proxqp::from_eigen, delta }, logger_ptr, qp, stack);
    } else {
                ...
                VectorViewMut<T> tmp_delta_preallocated) const
  {
    ...
    scale_qp_in_place(scaled_qp, tmp_delta_preallocated);
  }
  ...
  void scale_dual_in_place_eq(VectorViewMut<T> dual) const
  {
    dual.to_eigen().array() =
      ...
      delta.middleRows(dim, dual.to_eigen().size()).array();
  }
  /*!
  ...
  {
    dual.to_eigen().array() = dual.as_const().to_eigen().array() /
                              delta.tail(dual.to_eigen().size()).array();
  }
  /*!
  ...
  {
    dual.to_eigen().array() *= delta.head(dim).array();
  }
  /*!
...
#endif /* end of include guard PROXSUITE_PROXQP_DENSE_PRECOND_BUNCH_HPP */

...
#include <proxsuite/proxqp/dense/workspace_primal_dual_splitting_reduced.hpp>
...
void projection_onto_cones(PrimalDualSplittingWorkspace<T>& work,
    ...
    //work.u_u.head(n_eq).setZero();
    work.u_u.tail(n_in) = proxsuite::proxqp::dense::positive_part(work.tmp_y.tail(n_in));
    work.v_u.tail(n_in) += settings.tau * results.info.admm_step_size * (settings.alpha_over_relaxed * work.y_u - work.u_u.tail(n_in) + (1-settings.alpha_over_relaxed)*work.u_prev_u.tail(n_in) );
    //work.v_u.head(n_eq).setZero();
    //std::cout << "work.tmp_y.tail(n_in) " << work.tmp_y.tail(n_in) << std::endl;
    //std::cout << "work.u_u " << work.u_u << std::endl;
    ...
    work.u_l.tail(n_in) = proxsuite::proxqp::dense::positive_part(work.tmp_y.tail(n_in));
    work.v_l.tail(n_in) += settings.tau * results.info.admm_step_size * (settings.alpha_over_relaxed * work.y_l - work.u_l.tail(n_in) + (1-settings.alpha_over_relaxed)*work.u_prev_l.tail(n_in) );
    //std::cout << "work.u_l.tail(n_in) " << work.u_l.tail(n_in) << std::endl;
    //std::cout << "work.v_l.tail(n_in) " << work.v_l.tail(n_in) << std::endl;
  ...
  bool upper_part)
{
  ...
  if (upper_part){
    ...
    work.ldl.dual_solve_in_place(work.dw.tail(m),model.dim, stack);
	////std::cout << "G reconstructed " << work.ldl.dbg_reconstructed_matrix_low(model.dim,m) << std::endl;
	//std::cout << "dw " << work.dw.tail(m) << std::endl; 
		  ...
		  T mu_in_new)
{
  ...
  work.tmp_y.head(n_eq).setConstant(mu_eq_old - mu_eq_new);
  work.tmp_y.tail(n_in).setConstant(mu_in_old - mu_in_new);
  //std::cout <<" hello i do an update " << std::endl;
  {
...
void update_gk(PrimalDualSplittingWorkspace<T>& work,
	...
	work.g_k.tail(model.n_in) = work.C_scaled * work.dw.head(model.dim);
    //std::cout << "Aw_in " << work.g_k.tail(model.n_in) << std::endl;
	work.g_k.tail(model.n_in) += 0.5 * results.info.mu_in * work.y_prev.tail(model.n_in) ;
    //std::cout << "0.5 * results.info.mu_in * work.y_prev.tail(model.n_in)   "<< 0.5 * results.info.mu_in * work.y_prev.tail(model.n_in)  <<std::endl;
	work.g_k.tail(model.n_in) -= 0.5 * (work.u_scaled+work.l_scaled);
...
void update_y(PrimalDualSplittingWorkspace<T>& work,
    ...
    work.rhs.tail(n_in) = (- work.g_k.tail(n_in) +  0.5 * results.info.admm_step_size * (work.u_l.tail(n_in) - work.u_u.tail(n_in)) - 0.5 * (work.v_prev_l-work.v_prev_u));
    //std::cout << "work.rhs.tail(n_in) " << work.rhs.tail(n_in) << std::endl;
    //std::cout << "G reconstructed " << work.ldl.dbg_reconstructed_matrix_low(model.dim,m) << std::endl;
	socp_iterative_solve_with_permut_fact( //
    settings,
    model,
    ...
    work.y_l = 0.5 * (results.y.tail(n_in) + work.rhs.tail(n_in));
    work.y_u = work.y_l - results.y.tail(n_in) ;
...
void inner_residuals(PrimalDualSplittingWorkspace<T>& work,
    ...
    isize n_in = model.n_in;
    isize n_eq = model.n_eq;
    work.rhs.tail(n_eq+n_in) = work.v_l ;
	work.rhs.tail(n_in) -= work.v_u ;
    work.rhs.tail(n_in) *= 0.5 ;
    ...
    //std::cout << "Gy " << work.tmp_y << std::endl;
    //std::cout << "v " << work.rhs.tail(n_eq+n_in) << std::endl;
	work.tmp_y += work.g_k + work.rhs.tail(n_eq+n_in) ;
    work.tmp_y.head(n_eq) -= results.info.admm_step_size * results.y.head(n_eq);
    work.tmp_y.tail(n_in) -= 0.5 * results.info.admm_step_size * results.y.tail(n_in); // so we remove admm_step_size * y as we want the residuals without it 

...
struct PrimalDualSplittingWorkspace {
  ...
  VecBool active_set_bounded;
  VecBool active_set_lower_bounded;
  VecBool active_set_upper_bounded;
  VecBool active_set_unisides;
  ...
  PrimalDualSplittingWorkspace(isize dim, isize n_eq, isize n_in)
    ...
    , rhs(dim + n_eq + n_in)
    , err(dim + n_eq + n_in)
    , active_set_bounded(n_in)
    , active_set_lower_bounded(n_in)
    , active_set_upper_bounded(n_in)
    , active_set_unisides(n_in)
    , dw(dim + n_eq + n_in)
